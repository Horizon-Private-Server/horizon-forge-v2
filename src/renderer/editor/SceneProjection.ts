...
export class SceneProjection {
  ...
  sync(entities: readonly EditorEntity[]) {
    ...
    for (const entity of entities) {
      ...
      if (!object) {
        object = new THREE.Mesh(this.geometry, this.materialFor(entity));
      ...
      if (this.updateObject(object, entity)) updated += 1;
    }
  ...
  private updateObject(object: THREE.Mesh, entity: EditorEntity): boolean {
    const material = this.materialFor(entity);
  ...
  private materialFor(entity: EditorEntity): THREE.Material {
    return entity.asset ? this.assetMaterial : this.modelLessMaterial;
  }
}
